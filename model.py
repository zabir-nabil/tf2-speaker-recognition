...
class VladPooling(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        max_cluster_score = K.math.reduce_max(cluster_score, -1, keepdims=True)
        exp_cluster_score = K.math.exp(cluster_score - max_cluster_score)
        A = exp_cluster_score / K.math.reduce_sum(exp_cluster_score, axis=-1, keepdims = True)
        ...
        cluster_res = K.math.reduce_sum(weighted_res, [1, 2])

        if self.mode == 'gvlad':
            cluster_res = cluster_res[:, :self.k_centers, :]

        cluster_l2 = K.math.l2_normalize(cluster_res, -1)
