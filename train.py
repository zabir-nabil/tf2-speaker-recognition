...
np.seterr(all='raise') # raise error for all floating point
...
def main():
    ...
    #       Get Train/Val.
    # ==================================
    trnlist, trnlb = utils.get_voxceleb2_datalist(args, path=cfg.train_map["voxclb2_train"])
    vallist, vallb = utils.get_voxceleb2_datalist(args, path=cfg.train_map["voxclb2_val"])
...
def set_path(args):
    ...
    model_path = os.path.join('../tf2sr_models', exp_path)
    log_path = os.path.join('../tf2sr_logs', exp_path)
